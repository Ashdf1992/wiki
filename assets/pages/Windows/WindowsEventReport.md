...
## V2 (Powershell) - Checkbox to exclude 'Security' events
>Open Powershell or Powershell ISE

>Run the Following
```Powershell
# =========================
# NEW (Optional): GUI date/time + duration picker (WinForms) + Include Security checkbox
# =========================
function Show-EventWindowPicker {
    ...
    $form.Size = New-Object System.Drawing.Size(420, 270)   # slightly taller for the checkbox
    ...
    # === NEW: Include Security checkbox ===
    $chkSecurity = New-Object System.Windows.Forms.CheckBox
    $chkSecurity.Text = "Include Security events"
    $chkSecurity.Location = New-Object System.Drawing.Point(20, 135)
    $chkSecurity.AutoSize = $true
    $chkSecurity.Checked = $true   # default = include (matches original behaviour)

    $okButton = New-Object System.Windows.Forms.Button
    $okButton.Text = "OK"
    $okButton.Location = New-Object System.Drawing.Point(220, 180)
    $okButton.Width = 75
    $okButton.DialogResult = [System.Windows.Forms.DialogResult]::OK

    $cancelButton = New-Object System.Windows.Forms.Button
    $cancelButton.Text = "Cancel"
    $cancelButton.Location = New-Object System.Drawing.Point(305, 180)
    ...
    [pscustomobject]@{
        ...
        IncludeSecurity   = $chkSecurity.Checked
    }
}
...
if ($UseGuiPicker) {
    ...
    $IncludeSecurity = $picked.IncludeSecurity          # <-- new variable
}
else {
    # --- ORIGINAL INPUT PROMPTS (unchanged) ---
    # ... (your original Read-Host section remains exactly as it was)
    $DateInput = Read-Host "Enter date (yyyy-MM-dd) or press Enter for TODAY'S DATE"
    # ... rest of the else block unchanged ...
    $IncludeSecurity = $true   # default when using console prompts
}
...
# === MODIFIED: Define logs based on checkbox ===
if ($IncludeSecurity) {
    $AllLogs = @("Application", "System", "Security", "Setup")
} else {
    $AllLogs = @("Application", "System", "Setup")
}

Write-Host "`nRetrieving events between $StartDateTime and $EndDateTime..." -ForegroundColor Cyan
if (-not $IncludeSecurity) {
    Write-Host "Security log excluded per user selection." -ForegroundColor Gray
}

# The rest of your script (event collection, HTML export, etc.) remains 100% unchanged
$AllResults = @()
...
# =========================
# Export HTML Report (AUTOMATIC) - unchanged from here onwards
# =========================
# ... (your entire Export-ModernEventHtml function and the final export block stay exactly the same) }
}
